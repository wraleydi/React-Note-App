...
class NoteApp extends React.Component {
  ...
  render() {
    ...
    const searchNote = this.state.notes.filter((notes) => notes.title.toLowerCase().includes(this.state.search.toLowerCase()))
    ...
    <div className="note-app__body">
      <h2>Your Notes</h2>
      {activeNote.length > 0 ? (
        <NoteList notes={activeNote} onDelete={this.onDeleteHandler} onArchive={this.onArchiveHandler} />
      ) : (
        <MessageNote />
      )}
